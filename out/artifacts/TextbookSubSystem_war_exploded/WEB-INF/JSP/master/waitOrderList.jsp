                    ...
                    <table class="table table-striped">
                        ...
                        <c:forEach var="order" items="${orderList}">
                            <tr>
                                ...
                                <td>
                                    <button class="btn bg-success" onclick="passOrder(${order.id})">通过</button>
                                    <button class="btn bg-danger" onclick="unpassOrder(${order.id})">不通过</button>
                                </td>
                            </tr>
                        </c:forEach>
                    </table>
    ...
    function passOrder(id) {
        ...
        $.ajax({
            ...
            success: function (obj) {
                if (obj > 0) {
                    alert("已通过")
                    location.href = "<%=path%>/system/waitOrderList?userName=${user.userName}";
                } else {
                    alert("操作失败,请重试")
                    location.href = "<%=path%>/system/waitOrderList?userName=${user.userName}";
                }
